...
export default function AvailableBreed() {
  const [cats, setCats] = useState([]);
  const location = useLocation();
  console.log(location)
  const splitLocation = location.pathname.split('/');
  console.log(splitLocation)
  const filterredBreed = availableCats.filter((cat, i) => cat.breed.toLocaleLowerCase() === splitLocation[2])
  useEffect(() => {
  ...
  return (
    <section className="text-center mt-4">
      <h2>{splitLocation[2]} Cats</h2>
      <p>Meet our adorable cats looking for their forever home!</p>

      <div className="mt-2 mb-2 row g-4 cats-container flex" id="cats-container">
        {cats.length == 0 ? <h2>Sorry, {splitLocation[2]} Cats are not available </h2>
          :
          cats.map((cat, i) => (
